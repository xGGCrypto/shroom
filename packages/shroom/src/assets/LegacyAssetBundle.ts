  ...
  async getBlob(name: string, timeoutMs = 30000): Promise<Blob> {
    ...
    const fetchPromise = fetch(imageUrl)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(
            `Failed to fetch blob: ${imageUrl} - ${response.status}`
          );
        }
      ...
      .catch((err) => {
        console.error(
          `[LegacyAssetBundle] Error fetching blob '${name}':`,
          err
        );
        throw err;
      });
    const blob = withTimeout(
      fetchPromise,
      timeoutMs,
      `Fetching blob '${name}' timed out`
    );
  ...
  async getString(name: string, timeoutMs = 30000): Promise<string> {
    ...
    const fetchPromise = fetch(imageUrl)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(
            `Failed to fetch string: ${imageUrl} - ${response.status}`
          );
        }
      ...
      .catch((err) => {
        console.error(
          `[LegacyAssetBundle] Error fetching string '${name}':`,
          err
        );
        throw err;
      });
    const string = withTimeout(
      fetchPromise,
      timeoutMs,
      `Fetching string '${name}' timed out`
    );
